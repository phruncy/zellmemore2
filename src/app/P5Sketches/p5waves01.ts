...
export const waves01Factory = (automaton: AutomatonService) => {
    let agents: Agent[] = [];
    ...
    return (p5: widgetP5) => {
        const initValues = () => {
            ...
            linearWavelength = (p5.width - 2 * margin) / automaton.cellnumber;
            radius = p5.width / 2 - margin - 2 * amplitude;
            angle = (Math.PI * 2) / automaton.cellnumber;
            agents = [];
        ...
        const moveAgents = () => {
            const speed = (amplitude * 2) / (60 / automaton.fps());
        ...
        const drawLinear = () => {
            ...
            p5.beginShape();
            agents.forEach((agent) => {
                p5.curveVertex(agent.id * linearWavelength, -agent.pos);
            });
            p5.endShape();
        };
