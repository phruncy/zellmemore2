...
export const waves03Factory = (automaton: AutomatonService) => {
    ...
    return (p5: widgetP5) => {
        const initValues = () => {
            ...
            linearGap = (p5.width - 2 * margin) / automaton.cellnumber;
            radius = (p5.width - 2 * margin - 2 * amplitude) / 4;
            segment = (Math.PI * 2) / automaton.cellnumber;
        ...
        const moveAgents = () => {
            const speed = amplitude / (60 / automaton.fps());
